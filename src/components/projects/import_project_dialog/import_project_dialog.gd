...
#	super._ready()
	...
	_browse_project_path_button.pressed.connect(func():
		if _project_path_line_edit.text.is_empty():
			_browse_project_path_dialog.current_dir = ProjectSettings.globalize_path(
				Config.DEFAULT_PROJECTS_PATH.ret()
			)
		else:
			_browse_project_path_dialog.current_path = _project_path_line_edit.text
	...
	_project_path_line_edit.text_changed.connect(func(arg: String):
		_validate()
		_sort_options()
	)
...
func init(project_path, editor_options):
	...
	_project_path_line_edit.text = project_path if not project_path.is_empty() else ProjectSettings.globalize_path(
		Config.DEFAULT_PROJECTS_PATH.ret()
	)
	_validate()
	_sort_options()
...
func _sort_options():
	if _project_path_line_edit.text.get_file() in utils.PROJECT_CONFIG_FILENAMES:
		var cfg = Projects.ExternalProjectInfo.new(_project_path_line_edit.text)
		cfg.load(false)
