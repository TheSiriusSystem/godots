...
func install_zip(zip_reader: ZIPReader, project_name):
	...
	_install_project_from_zip_dialog.about_to_install.connect(func(final_project_name, project_dir, editor_path):
		...
		zip_reader.close()
		if unzip_err != OK:
			_install_project_from_zip_dialog.error(tr("Failed to unzip project."))
			return
		var project_configs = utils.find_project_configs(project_dir)
		if len(project_configs) == 0:
			_install_project_from_zip_dialog.error(tr("No project configuration file found."))
			return
...
func _scan_projects(dir_path):
	var project_configs = utils.find_project_configs(dir_path)
	var added_projects = []
...
func _on_projects_list_item_duplicate_requested(project: Projects.Item) -> void:
	...
	_duplicate_project_dialog.about_to_install.connect(func(final_project_name, project_dir):
		...
		var project_configs = utils.find_project_configs(project_dir)
		if len(project_configs) == 0:
			_duplicate_project_dialog.error(tr("No project configuration file found."))
			return
